...
class SearchIndexLiveValidationGuardTests(unittest.TestCase):
    def _module(self, *, html, page_ean=EAN, status_code=200, hint=1299.99):
        watch = [{
            "url": URL,
        ...
        module = SimpleNamespace(
            ...
            page_identifiers=lambda soup: {"ean": page_ean} if page_ean else {},
    ...
    def _high_html(self, price="1299.99"):
        ...
        </head><body><h1>ASUS TUF Gaming</h1></body></html>
        """
    ...
    def test_strong_ean_and_high_live_price_promotes_candidate(self):
        ...
        self.assertEqual(items[0]["detail_source"], "search_index_live_validation")
        self.assertEqual(items[0]["price_page_confidence"], "HIGH")
        row = stat["search_index"]["watchlist"][0]
        self.assertEqual(row["price_status"], "LIVE_HIGH")
        self.assertEqual(row["live_validation_status"], "validated")
    ...
    def test_identity_mismatch_never_creates_candidate(self):
        ...
        self.assertEqual(row["live_validation_status"], "identity_mismatch")
    ...
    def test_single_price_family_stays_index_only(self):
        ...
        self.assertEqual(items, [])
        self.assertEqual(
            stat["search_index"]["watchlist"][0]["live_validation_status"],
            "price_not_high",
        )
