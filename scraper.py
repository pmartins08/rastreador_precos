...
GPU_MODELOS = [
    ...
    "rtx a5500", "rtx a5000", "rtx a4500", "rtx a3000", "rtx a2000",
    "radeon rx 7900m", "radeon rx 7800m", "radeon rx 7700s", "radeon rx 7600s",
    "radeon rx 7600m xt", "radeon rx 7600m", "radeon rx 6850m xt", "radeon rx 6800m",
    "radeon rx 6650m", "radeon rx 6600m", "radeon rx 6550m", "radeon rx 6500m",
]
GPU_MODELOS_ORDENADOS = sorted(GPU_MODELOS, key=len, reverse=True)
GPU_DEDICADA_GENERICA = [
    "rtx graphics", "rtx discrete", "geforce rtx", "geforce mx",
    "radeon rx", "radeon pro", "radeon 6xxxm", "radeon 7xxxm",
]
GPU_INTEGRADA_TERMS = [
    "intel iris", "intel arc graphics", "intel graphics", "intel uhd",
    "intel xe", "intel xe graphics", "intel arc integrated",
    "radeon graphics", "radeon 610m", "radeon 680m", "radeon 780m",
    "radeon 840m", "radeon 890m", "radeon 760m", "amd radeon graphics",
    "amd integrated graphics", "qualcomm adreno", "adreno", "qualcomm gpu",
]
GPU_PONTOS = {
    ...
    "rtx a5500": 96, "rtx a5000": 92, "rtx a4500": 82, "rtx a3000": 68, "rtx a2000": 55,
    "radeon rx 7900m": 96, "radeon rx 7800m": 88, "radeon rx 7700s": 80, "radeon rx 7600s": 68,
    "radeon rx 7600m xt": 72, "radeon rx 7600m": 65, "radeon rx 6850m xt": 78,
    "radeon rx 6800m": 76, "radeon rx 6650m": 60, "radeon rx 6600m": 56,
    "radeon rx 6550m": 48, "radeon rx 6500m": 42,
}
GPU_PRECO_MIN = {
    "rtx 5090": 1600, "rtx 5080": 1250, "rtx 5070 ti": 1000, "rtx 5070": 850,
    "rtx 5060 ti": 700, "rtx 5060": 600, "rtx 5050": 500, "rtx 4090": 1400,
    "rtx 4080": 1200, "rtx 4070": 850, "rtx 4060": 650, "rtx 4050": 500,
    "rtx a5500": 1300, "rtx a5000": 1200, "rtx a4500": 900, "rtx a3000": 650, "rtx a2000": 550,
    "radeon rx 7900m": 1100, "radeon rx 7800m": 850, "radeon rx 7700s": 750, "radeon rx 7600s": 650,
    "radeon rx 7600m xt": 700, "radeon rx 7600m": 600, "radeon rx 6850m xt": 750,
    "radeon rx 6800m": 750, "radeon rx 6650m": 550, "radeon rx 6600m": 500,
    "radeon rx 6550m": 450, "radeon rx 6500m": 400,
}
...
def extrair_specs_avancadas(texto_bruto: str) -> dict:
    ...
    s = {
        ...
        "gpu_modelos_detectados": [], "cpu_modelo": None, "cpu_classe": None,
        "cpu_str_original": None, "ram_gb": None, "ram_expansivel": False,
        "armazenamento_tb": None, "ssd_expansivel": False, "bateria_wh": None,
        "peso_kg": None, "ecra_res": None, "ecra_hz": None,
        "teclado_pt": "desconhecido", "alertas": [], "fontes": {},
    }
    s["marca"], s["submarca"] = detetar_marca_submarca(t)
    if s["marca"]:
        s["fontes"]["marca"] = "titulo"

    detected_dedicated = [gpu for gpu in GPU_MODELOS_ORDENADOS if gpu in t]
    if detected_dedicated:
        s["gpu_modelo"] = detected_dedicated[0]
        s["gpu_tipo"] = "dedicada"
        s["gpu_modelos_detectados"] = detected_dedicated
        s["fontes"]["gpu"] = "modelo_exato"
    elif any(x in t for x in GPU_DEDICADA_GENERICA):
        s["gpu_tipo"] = "dedicada"
        s["fontes"]["gpu"] = "dedicada_generica"
        s["alertas"].append("GPU dedicada detetada, modelo não identificado")
    elif any(x in t for x in GPU_INTEGRADA_TERMS):
...
def calcular_scores(s: dict, preco: float, weights: dict, settings: dict) -> dict:
    ...
    if s.get("gpu_tipo") == "integrada":
        return {"status": "REJEITADO", "alertas": ["GPU apenas integrada — excluído para gaming/modelação 3D."]}
    if s["ram_gb"] == 8:
    ...
    p_gpu = gpu.get(s["gpu_modelo"], 45) if s["gpu_tipo"] == "dedicada" else 30
    peso = s.get("peso_kg")
    ...
    if s.get("gpu_tipo") == "desconhecida":
        alertas.append("GPU não confirmada — não será tratada como dedicada.")
    return {
        ...
        "detalhes": {
            ...
            "gpu_tipo": s.get("gpu_tipo"), "cpu": s.get("cpu_str_original"), "ram_gb": s.get("ram_gb"),
...
def extrair_candidatos_html(soup: BeautifulSoup, cfg: dict, base_url: str, limit: int) -> list[dict]:
    ...
    for anchor in soup.select("a[href]"):
        ...
        hinted = any(h.lower() in href for h in hints)
        obvious_non_product = any(part in href for part in [
            "/login", "/conta", "/carrinho", "/checkout", "/wishlist", "/pesquisa", "/search", "javascript:", "mailto:"
        ])
        if not hinted and obvious_non_product:
            continue
        parent = anchor
        best = None
...
def extrair_grelha_categoria(session: requests.Session, cfg: dict, limit: int) -> dict:
    ...
    blocked = status in (401, 403, 429, 503) or any(x in title.lower() for x in ["just a moment", "access denied", "verify you are human"]) or "cf-chl-" in response.text.lower()
    ...
    for item in jsonld + html_items:
        key = normalizar_texto(item.get("titulo", ""))
        ...
        if len(merged) >= limit:
            break
    if not merged:
        print(f"   ⚠️ {loja}: acesso OK mas sem produtos extraídos.")
    return {"loja": loja, "url": response.url, "produtos": merged, "bloqueada": False, "erro": None}


def extrair_texto_detalhe(html: str, base_url: str = "") -> str:
    soup = BeautifulSoup(html, "html.parser")
    partes = []
    if soup.title:
        partes.append(soup.title.get_text(" ", strip=True))
    for node in soup.select("meta[name='description'], meta[property='og:title'], meta[property='og:description']"):
        content = (node.get("content") or "").strip()
        if content:
            partes.append(content)
    partes.append(soup.get_text(" ", strip=True))
    for item in extrair_jsonld_produtos(soup, base_url):
        partes.append(item.get("titulo", ""))
...
def enriquecer_item(session: requests.Session, item: dict) -> dict:
    specs = item.get("_specs")
    if not specs or not validar_url_produto(item):
        return item
    if not any([specs.get("gpu_tipo") == "desconhecida", not specs.get("cpu_modelo"), specs.get("ram_gb") is None]):
        return item
    try:
        response = session.get(item["url"], timeout=9, allow_redirects=True, headers={"Referer": item.get("_category_url", "")})
        if response.status_code >= 400:
            return item
        detail_text = extrair_texto_detalhe(response.text, response.url)
        enriched = extrair_specs_avancadas(" ".join([item.get("titulo", ""), detail_text]))
        for field in [
            "gpu_modelo", "gpu_tipo", "cpu_modelo", "cpu_classe", "cpu_str_original", "ram_gb",
            "ram_expansivel", "armazenamento_tb", "ssd_expansivel", "bateria_wh", "peso_kg",
            "ecra_res", "ecra_hz", "teclado_pt",
        ]:
            current = specs.get(field)
            candidate = enriched.get(field)
            empty = current in (None, "desconhecida", "desconhecido", False)
            if empty and candidate not in (None, "desconhecida", "desconhecido", False):
                specs[field] = candidate
                specs.setdefault("fontes", {})[field] = "pagina_produto"
        if enriched.get("gpu_modelo") and not specs.get("gpu_modelo"):
            specs["gpu_modelo"] = enriched["gpu_modelo"]
            specs["gpu_tipo"] = "dedicada"
            specs.setdefault("fontes", {})["gpu"] = "pagina_produto"
        specs.setdefault("fontes", {})["detalhe_produto"] = "pagina_produto"
        return item
...
def detetar_teclado_html(html: str) -> str:
    soup = BeautifulSoup(html, "html.parser")
    campos = [node.get_text(" ", strip=True) for node in soup.select("[class*='keyboard'], [class*='Keyboard'], [class*='teclado'], [class*='Teclado'], [itemprop*='keyboard'], [data-testid*='keyboard']")]
...
def confirmar_teclado_produto(session: requests.Session, item: dict) -> str:
    ...
    try:
        ...
        return detetar_teclado_html(response.text) if response.status_code < 400 else "desconhecido"
    except requests.RequestException:
        return "desconhecido"
...
def enviar_alerta(titulo: str, mensagem: str, prioridade: str = "default", tags: str = "computer") -> None:
    ...
    try:
        requests.post(
            "https://ntfy.sh",
            json={"topic": NTFY_TOPIC, "title": titulo, "message": mensagem, "tags": [t.strip() for t in tags.split(",") if t.strip()], "priority": 4 if prioridade == "high" else 3},
            timeout=10,
        ).raise_for_status()
...
def formatar_alerta(tier: str, item: dict, analysis: dict, specs: dict, reason: str) -> tuple[str, str, str]:
    ...
    tags = {"DIAMANTE": "gem", "OURO": "trophy", "PRATA": "medal_sports", "BRONZE": "medal"}
    title = f"{icons.get(tier, '💻')} {tier} | {item['preco']:.0f}€"
    msg = (
        f"{item['titulo']}\n\nLoja: {item['loja']}\nPreço: {item['preco']:.2f}€\n"
        ...
        f"GPU: {specs.get('gpu_modelo') or 'não identificada'} ({specs.get('gpu_tipo')})\n"
        f"CPU: {specs.get('cpu_str_original') or 'não confirmada'}\nRAM: {specs.get('ram_gb') or '?'}GB\n"
        f"Teclado: {specs['teclado_pt']}\nMotivo: {reason}\n\n🔗 Produto: {item['url']}"
    )
    return title, msg, tags.get(tier, "computer")


def main() -> None:
    started = datetime.now(timezone.utc)
    print("🚀 Rastreador iniciado — GPU integrada-only excluída; ranking 0–100; tiers configuráveis.")
    ...
    enrich_limit = max(0, int(settings.get("max_enriquecimentos_detalhe", 18)))
    stats = {
        "lojas": len(categories), "lojas_com_produtos": 0, "lojas_sem_resultados": 0,
        "bloqueadas": 0, "erros": 0, "encontrados": 0, "analisados": 0, "aceites": 0,
        "rejeitados": 0, "rejeitados_gpu_integrada": 0, "gpu_desconhecida": 0, "enriquecidos": 0,
        "precos_suspeitos": 0, "alertas": 0, "diamante": 0, "ouro": 0, "prata": 0, "bronze": 0,
    }

    session = requests.Session()
    session.headers.update({
        "User-Agent": USER_AGENT, "Accept-Language": "pt-PT,pt;q=0.9,en;q=0.7",
    ...
    with ThreadPoolExecutor(max_workers=max_workers or 1) as executor:
        ...
        result_map = {future_map[f]: f.result() for f in as_completed(future_map)}
    ...
    for category in categories:
        ...
        for item in products:
            ...
            if not ok_price:
                ...
                continue
            if specs.get("gpu_tipo") == "integrada":
                stats["rejeitados"] += 1
                stats["rejeitados_gpu_integrada"] += 1
                print(f"   [-] {item['titulo'][:68]}... | REJEITADO: GPU apenas integrada.")
                continue
            if specs.get("gpu_tipo") == "desconhecida":
                stats["gpu_desconhecida"] += 1
    ...
    def check_keyboard(entry):
        ...
        if detected != "desconhecido":
            ...
            specs.setdefault("fontes", {})["teclado"] = "pagina_produto"
    ...
    enrich_candidates = []
    for category in categories:
        for item in result_map.get(category["loja"], {}).get("produtos", []):
            specs = item.get("_specs")
            if specs and (specs.get("gpu_tipo") == "desconhecida" or not specs.get("cpu_modelo") or specs.get("ram_gb") is None):
                enrich_candidates.append(item)
    enrich_candidates = enrich_candidates[:enrich_limit]
    if enrich_candidates:
        print(f"   🔎 Enriquecimento de detalhe: {len(enrich_candidates)} produtos.")
        with ThreadPoolExecutor(max_workers=min(max_workers or 1, 6)) as executor:
            futures = [executor.submit(enriquecer_item, session, item) for item in enrich_candidates]
            for future in as_completed(futures):
                future.result()
        stats["enriquecidos"] = sum(1 for item in enrich_candidates if item.get("_specs", {}).get("fontes", {}).get("detalhe_produto") == "pagina_produto")

    for category in categories:
        loja = category["loja"]
        for item in result_map.get(loja, {}).get("produtos", []):
            specs = item.get("_specs")
            if not specs:
                continue
            ...
            record = {
                ...
                "oportunidade": tier, "qualidade_dados": analysis["qualidade_dados"], "teclado_pt": specs["teclado_pt"],
                "marca": specs.get("marca"), "submarca": specs.get("submarca"), "gpu": specs.get("gpu_modelo"),
                "gpu_tipo": specs.get("gpu_tipo"), "cpu": specs.get("cpu_str_original"), "url": item["url"],
            }
            entries.append(record)
            history["offers"][key] = entries[-60:]
            print(f"   [+] {item['titulo'][:48]}... | {item['preco']:.2f}€ | Ranking: {analysis['score_ranking']}/100 | Valor: {analysis['value_score']} | {tier or 'sem tier'} | Dados: {analysis['qualidade_dados']} | GPU: {specs.get('gpu_tipo')}")
    ...
    resumo = (
        ...
        f"GPU integrada rejeitada: {stats['rejeitados_gpu_integrada']}\nGPU desconhecida: {stats['gpu_desconhecida']}\n"
        f"Enriquecidos na página do produto: {stats['enriquecidos']}\nPreços suspeitos descartados: {stats['precos_suspeitos']}\n"
