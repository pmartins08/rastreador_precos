...
def blank_learning() -> dict:
    return {"schema_version": 2, "updated_at": now_iso(), "stores": {}}


LEARNING = load_json(LEARNING_PATH)
if LEARNING.get("schema_version") != 2:
    old_stores = LEARNING.get("stores", {}) if isinstance(LEARNING, dict) else {}
    LEARNING = blank_learning()
    for store, old in old_stores.items():
        if not isinstance(old, dict):
            continue
        bucket = {
            "attempts": int(old.get("attempts", 0)),
            "successes": int(old.get("successes", 0)),
            "blocks": int(old.get("blocks", 0)),
            "errors": old.get("errors", {}) if isinstance(old.get("errors"), dict) else {},
            "profiles": old.get("profiles", {}) if isinstance(old.get("profiles"), dict) else {},
            "methods": old.get("methods", {}) if isinstance(old.get("methods"), dict) else {},
            "contexts": {},
            "last_updated": old.get("last_updated"),
        }
        for profile, stats in bucket["profiles"].items():
            bucket["contexts"].setdefault("legacy", {})[profile] = {
        ...
        LEARNING["stores"][store] = bucket


def bucket(store: str) -> dict:
    with LOCK:
        return LEARNING["stores"].setdefault(store, {
            "attempts": 0, "successes": 0, "blocks": 0, "errors": {},
            "profiles": {}, "methods": {}, "contexts": {}, "last_updated": None,
        })
...
def record_learning(store: str, profile: str, outcome: str, method: str) -> None:
    with LOCK:
        ...
        c = b["contexts"].setdefault(method, {}).setdefault(profile, {"attempts": 0, "successes": 0, "blocks": 0, "errors": 0, "results": {}})
...
def profile_order(store: str, method: str) -> list[str]:
    ...
    for i, profile in enumerate(PROFILES):
        ...
        if attempts == 0:
            # Exploration controlada: cada perfil novo é testado pelo menos uma vez.
            key = (0, i)
        else:
            successes = int(c.get("successes", 0)); blocks = int(c.get("blocks", 0))
            rate = (successes + 1.0) / (attempts + 2.0)
            penalty = blocks / max(1, attempts)
            key = (-(rate - 0.35 * penalty), attempts, i)
        ranked.append((key, profile))
    ranked.sort()
...
def header_for(profile: str) -> dict:
    ...
    if profile.startswith("firefox"):
        ...
        return {"User-Agent": f"Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:{v}.0) Gecko/20100101 Firefox/{v}.0", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8", "Accept-Language": "pt-PT,pt;q=0.9,en-US;q=0.8"}
    if profile.startswith("edge"):
        v = profile.removeprefix("edge")
        return {"User-Agent": f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{v}.0.0.0 Safari/537.36 Edg/{v}.0.0.0", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8", "Accept-Language": "pt-PT,pt;q=0.9,en-US;q=0.8"}
    v = profile.removeprefix("chrome")
    return {"User-Agent": f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{v}.0.0.0 Safari/537.36", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8", "Accept-Language": "pt-PT,pt;q=0.9,en-US;q=0.8"}
...
def classify_response(response) -> tuple[str, bool]:
    ...
    try:
        ...
        if any(x in title or x in visible for x in markers) or "cf-chl-" in title or "captcha" in title:
...
def retry_delay(attempt: int, response=None) -> float:
    ...
    if response is not None:
        ...
        try: retry_after = min(30.0, float(raw)) if raw else 0.0
        except (TypeError, ValueError): pass
    return max(retry_after, base + random.uniform(0.15, 0.65))
    ...
    for profile in profile_order(store, method):
        for attempt in range(2):
            ...
            try:
                ...
                last = response; outcome, retryable = classify_response(response)
                if outcome == "http_success":
                    record_learning(store, profile, "success", method); return response, profile, outcome
            ...
            if attempt == 0: time.sleep(retry_delay(attempt, response))
    return last, None, classify_response(last)[0] if last is not None else "no_response"
...
def discover_sitemaps(cat: dict, config: dict, max_urls: int = 80, max_indexes: int = 20) -> list[str]:
    ...
    robots, _, _ = adaptive_fetch(urljoin(origin, "/robots.txt"), config, 10)
    ...
    while seeds and len(out) < max_urls and len(seen_sm) < max_indexes:
        ...
        seen_sm.add(sm)
        response, _, _ = adaptive_fetch(sm, config, 12)
...
def alert_payload(tier: str, item: dict, assessment: dict, reason: str):
    ...
    s = item.get("specs") or {}; value = assessment["value_score"]; rank = assessment["score_ranking"]
    if tier == "DIAMANTE":
        msg = (f"{item['titulo']}\nLoja: {item['loja']}\nPreço: {item['preco']:.2f}€\n"
               f"Value: {value:.1f} | Rank: {rank:.1f}\nMotivo: {reason}\n"
               f"GPU: {s.get('gpu_modelo') or s.get('gpu_tipo')}\nCPU: {s.get('cpu_modelo') or '?'}\n"
               f"RAM: {s.get('ram_gb') or '?'}GB | SSD: {s.get('armazenamento_tb') or '?'}TB\n"
               f"Ecrã: {s.get('ecra_res') or '?'} {s.get('ecra_hz') or '?'}Hz\n"
               f"Bateria: {s.get('bateria_wh') or '?'}Wh | Peso: {s.get('peso_kg') or '?'}kg\n{item['url']}")
        return title, msg, "high"
    if tier == "OURO":
        msg = (f"{item['titulo']}\nLoja: {item['loja']} | Preço: {item['preco']:.2f}€\n"
               f"Value: {value:.1f} | Rank: {rank:.1f}\nMotivo: {reason}\n"
               f"GPU: {s.get('gpu_modelo') or s.get('gpu_tipo')} | CPU: {s.get('cpu_modelo') or '?'}\n"
               f"RAM: {s.get('ram_gb') or '?'}GB | SSD: {s.get('armazenamento_tb') or '?'}TB\n{item['url']}")
...
def alert_send(title: str, message: str, priority: str = "default") -> bool:
    ...
    if not topic: LOGGER.warning("NTFY_TOPIC não definido."); return False
    priority_map = {"high": 5, "default": 3, "low": 2, "min": 1}
    payload = {"topic": topic, "title": title[:180], "message": message, "tags": ["computer", "rotating_light"], "priority": priority_map.get(priority, 3)}
...
def main() -> None:
    ...
    for cat in config.get("category_urls", []):
        store = cat["loja"]; stats[store] = {"encontrados": 0, "candidatos": 0, "enriquecidos": 0, "aceites": 0, "rejeitados": 0, "bloqueada": False, "acesso": {}}
    ...
    for item in enriched:
        ...
        if tier and assessment["value_score"] >= min_alert and item.get("stock") is not False:
            ...
            price_drop = base is not None and price <= base - drop; opportunity = baseline_value < min_alert; upgrade = bool(old_tier and tier_order.get(tier, 0) > tier_order.get(old_tier, 0)); first = prior is None; should = first or opportunity or upgrade or price_drop
            can_push = tier_order.get(tier, 0) >= tier_order.get(min_notify_tier, 3)
            if should and can_push:
                ...
                if alert_send(title, message, priority):
                    alerts += 1; history["alert_state"][alert_key] = {"timestamp": now_iso(), "price": price, "value_score": assessment["value_score"], "tier": tier}
            elif should:
                suppressed += 1; LOGGER.info("Notificação suprimida (%s): %s | Value %.1f", tier, item["titulo"], assessment["value_score"])
    ...
    for store, stat in stats.items(): LOGGER.info("🏪 %s: encontrados=%d candidatos=%d enriquecidos=%d aceites=%d rejeitados=%d bloqueada=%s erro=%s acesso=%s", store, stat["encontrados"], stat["candidatos"], stat["enriquecidos"], stat["aceites"], stat["rejeitados"], stat["bloqueada"], errors.get(store), stat["acesso"])
