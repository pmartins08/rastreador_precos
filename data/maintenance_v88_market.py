...
old_cache = '''def latest_specs_by_url(history: dict) -> dict[str, dict]:
    out = {}
    ...
    return out
'''
new_cache = '''PRICE_EVIDENCE_FIELDS = {
    "price_confirmed",
...
def latest_specs_by_url(history: dict) -> dict[str, dict]:
    ...
    return out
'''
tracker = replace_once(tracker, old_cache, new_cache, "latest_specs_by_url")

anchor = '''def select_with_cache(
    items: list[dict],
'''
market_helpers = '''def _price_close(left: float, right: float, settings: dict) -> bool:
...
'''
tracker = replace_once(tracker, anchor, market_helpers + anchor, "market helper anchor")

old_scoring_start = '''    tiers = {"DIAMANTE": 0, "OURO": 0, "PRATA": 0, "BRONZE": 0}
    alerts = 0
    suppressed = 0
    current_ranked = []
    match_records = []
...
'''
new_scoring_start = '''    preliminary_records = []
    for item in evaluated:
...
'''
tracker = replace_once(tracker, old_scoring_start, new_scoring_start, "scoring prepass")

old_spec = '''        spec = item.get("specs") or scraper.specs(item.get("titulo", ""))
        assessment = score_allow_unknown(spec, float(price), weights, settings)
'''
new_spec = '''        assessment = score_allow_unknown(spec, float(price), weights, settings)
'''
tracker = replace_once(tracker, old_spec, new_spec, "scoring spec line")
tracker_path.write_text(tracker, encoding="utf-8")


guard_path = Path("price_guard.py")
guard = guard_path.read_text(encoding="utf-8")
old_start = '''    sources = list(spec.get("price_evidence_sources") or [])
    reason = _suspicion_reason(spec, candidate)

    if confirmed is not None:
'''
new_start = '''    sources = list(spec.get("price_evidence_sources") or [])
    ...
    if confirmed is not None:
'''
guard = replace_once(guard, old_start, new_start, "validate market vars")
...
new_insert = '''                ),
    ...
    if reason:
'''
guard = replace_once(guard, old_insert, new_insert, "market conflict")

old_suspicious = '''    if reason:
...
'''
guard = replace_once(guard, old_suspicious, new_suspicious, "suspicious confirmation")

old_meta = '''            result["price_evidence_sources"] = list(spec.get("price_evidence_sources") or [])
            result["exceptional_deal_bonus"] = bonus
'''
new_meta = '''            result["price_evidence_sources"] = list(spec.get("price_evidence_sources") or [])
            ...
            result["exceptional_deal_bonus"] = bonus
'''
guard = replace_once(guard, old_meta, new_meta, "market score metadata")
guard_path.write_text(guard, encoding="utf-8")


test_path = Path("tests/test_price_guard.py")
...
if insert.strip() not in tests:
    tests = replace_once(tests, marker, "\n" + insert + marker, "price guard test marker")
test_path.write_text(tests, encoding="utf-8")

tracker_tests_path = Path("tests/test_tracker.py")
tracker_tests = tracker_tests_path.read_text(encoding="utf-8")
tracker_insert = '''
...
'''
if tracker_insert.strip() not in tracker_tests:
    tracker_tests = replace_once(tracker_tests, marker, "\n" + tracker_insert + marker, "tracker test marker")
tracker_tests_path.write_text(tracker_tests, encoding="utf-8")
