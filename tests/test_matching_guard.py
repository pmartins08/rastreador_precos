        ...
        for left in range(len(records)):
            for right in range(left + 1, len(records)):
                result = self.match_configurations(
                    records[left]["item"], records[left]["spec"],
                    records[right]["item"], records[right]["spec"],
                )
                if result["level"] in counts:
                    counts[result["level"]] += 1
        return {
            "exact_pairs": counts["EXATO"], "strong_pairs": counts["FORTE"],
            "probable_pairs": counts["PROVAVEL"], "conflicting_pairs": counts["NAO_FUNDIR"],
            "groups": [], "probable_review": [],
        }
...
class MatchingGuardTests(unittest.TestCase):
    ...
    def test_same_ean_never_overrides_gpu_conflict(self):
        ...
        result = tracker.match_configurations(records[0]["item"], records[0]["spec"], records[1]["item"], records[1]["spec"])
    ...
    def test_legacy_intel_core_aliases_are_same_cpu(self):
        tracker = self._tracker()
        left = {"ean": "4711387967997", "loja": "Globaldata"}
        right = {"ean": "4711387967997", "loja": "UPTECHBOX"}
        result = tracker.match_configurations(
            left, {"cpu_modelo": "i7-13620h", "gpu_modelo": "rtx 4050", "ram_gb": 32},
            right, {"cpu_modelo": "core i7-13620h", "gpu_modelo": "rtx 4050", "ram_gb": 32},
        )
        self.assertEqual(result["level"], "EXATO")

    def test_core_7_and_ryzen_ai_7_never_collapse_to_same_cpu(self):
        tracker = self._tracker()
        result = tracker.match_configurations(
            {"ean": "0199276826169"}, {"cpu_modelo": "core 7 350", "ram_gb": 16},
            {"ean": "0199276826169"}, {"cpu_modelo": "ryzen ai 7 350", "ram_gb": 16},
        )
        self.assertEqual(result["level"], "NAO_FUNDIR")
        self.assertEqual(result["conflicting_fields"], ["cpu_modelo"])
    ...
    def test_missing_spec_does_not_create_false_conflict(self):
        tracker = self._tracker()
        result = tracker.match_configurations(
            {"ean": "1234567890123"}, {"gpu_modelo": "rtx 5050", "ram_gb": 32},
            {"ean": "1234567890123"}, {"gpu_modelo": None, "ram_gb": 32},
        )
        self.assertEqual(result["level"], "EXATO")
    ...
    def test_reversed_numeric_resolution_is_same_panel(self):
        tracker = self._tracker()
        result = tracker.match_configurations(
            {"ean": "0199275936708"}, {"ecra_res": "2560x1600", "ram_gb": 32, "armazenamento_tb": 1.0},
            {"ean": "0199275936708"}, {"ecra_res": "1600 x 2560 pixels", "ram_gb": 32, "armazenamento_tb": 1.0},
        )
        self.assertEqual(result["level"], "EXATO")
    ...
    def test_genuinely_different_numeric_resolution_still_conflicts(self):
        tracker = self._tracker()
        result = tracker.match_configurations(
            {"ean": "1234567890123"}, {"ecra_res": "1920x1080"},
            {"ean": "1234567890123"}, {"ecra_res": "2560x1600"},
        )
        self.assertEqual(result["level"], "NAO_FUNDIR")
    ...
    def test_resolution_orientation_does_not_block_market_evidence(self):
        tracker = self._tracker()
        records = [
            {"item": {"ean": "0199275936715", "loja": "A"}, "spec": {"ecra_res": "2560 × 1600", "ram_gb": 16, "armazenamento_tb": 1.0}},
            {"item": {"ean": "0199275936715", "loja": "B"}, "spec": {"ecra_res": "1600 x 2560 pixels", "ram_gb": 16, "armazenamento_tb": 1.0}},
        ]
    ...
    def test_market_evidence_is_blocked_for_ambiguous_ean(self):
        ...
        for record in records:
            ...
            self.assertEqual(record["spec"]["market_identity_conflict_fields"], ["gpu_modelo"])
    ...
    def test_build_cross_store_matching_counts_identifier_conflict(self):
        tracker = self._tracker()
        result = tracker.build_cross_store_matches(self._conflicting_records())
    ...
    def test_matching_state_is_derived_and_json_serializable(self):
        tracker = DummyTracker()
        records = [{
            "item": {"ean": "1234567890123", "loja": "A", "url": "https://a/item", "titulo": "Laptop A", "preco": 999.9},
            "spec": {"cpu_modelo": "ryzen 7 260", "gpu_modelo": "rtx 5050", "ram_gb": 32,
                     "armazenamento_tb": 1.0, "ecra_res": "2560x1600", "ecra_hz": 165},
        }]
        matching = {"exact_pairs": 0, "strong_pairs": 0, "probable_pairs": 0, "conflicting_pairs": 0, "groups": [], "probable_review": []}
